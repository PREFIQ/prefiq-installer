...
def install_package():
    top_level = next(INSTALL_DIR.glob("prefiq-*"), None)
    if not top_level or not top_level.is_dir():
        log("Error: Extracted source folder not found.")
        sys.exit(1)

    prefiq_dir_root = None
    for root, dirs, _ in os.walk(top_level):
        if "prefiq" in dirs:
            prefiq_dir_root = Path(root)
            break

    if not prefiq_dir_root:
        log("Error: Could not find the 'prefiq' source folder after extraction.")
        sys.exit(1)

    verify_required_folders(prefiq_dir_root)
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "-e", str(prefiq_dir_root)])
